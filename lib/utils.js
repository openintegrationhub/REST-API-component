...
module.exports.processMethod = async function (msg, cfg, snapshot, TOKEN) {
  ...
  async function buildErrorStructure(e) {
    ...
    if (
      ...
      const logContext = addErrorLogContext(e);
      emitter.logger.info(logContext, 'Starting rebound, Component error: %s', JSON.stringify(e));
      ...
      const logContext = addErrorLogContext(e);
      emitter.logger.debug(logContext, 'Component output: %o', returnOutput);
      await emitter.emit('data', messages.newMessage(returnOutput));
      await rateLimit(emitter.logger, rateLimitDelay);
    } else {
      const logContext = addErrorLogContext(e);
      emitter.logger.error(logContext, 'Component error: %s', JSON.stringify(e));
