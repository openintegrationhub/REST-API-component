  ...
  describe("oauth2 credentials", () => {
    ...
    it("should fail if auth.oauth2.keys is missing", async () => {
      ...
      try {
        ...
        expect(err.message).equal(
          "Error: OAuth2 provider hasn`t returned keys for current credentials"
        );
      }
    });
    ...
    it("should fail if auth.oauth2.keys.access_token is missing", async () => {
      ...
      try {
        ...
        expect(err.message).equal(
          'Error: No access tokens were returned by the OAuth2 provider: {"error":"invalid_client_secret"}'
        );
      }
    });
    ...
    it("should fail if auth.oauth2.keys.refresh_token is missing", async () => {
      ...
      try {
        ...
        expect(err.message).equal(
          "Error: No refresh tokens were returned by the OAuth2 provider. Try to add access_type:offline as an additional parameter"
        );
      }
    });
  });
